...
function App(){
  ...
  useEffect(()=>{
    try{
      const saved=JSON.parse(localStorage.getItem('dancePromptStudioConfig')||'{}');
      if(saved.apiKeys) setApiKeys(saved.apiKeys);
      if(saved.chunk) setChunk(saved.chunk);
  ...
  useEffect(()=>{
    localStorage.setItem('dancePromptStudioConfig', JSON.stringify({apiKeys, chunk, platform, modelName, extra}));
  },[apiKeys, chunk, platform, modelName, extra]);
